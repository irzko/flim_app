    ...
    .then((response) => {
      ...
      if (response.data.length === 0) {
        ...
        response.data.forEach(async (theatre) => {
          ...
          for (let i = 0; i < showtime.data.length; i++) {
            theatre_id.push(showtime.data[i].theatre_id);
          }
          ...
          for (let i = 0; i < timeBtn.length; i++) {
            timeBtn[i].addEventListener("click", (event) => {
              if (username) {
                const time = {
                  movie_id: movie_id,
                  theatre_id: theatre_id[i],
                  start_time: `${dateSelected[2]}-${dateSelected[1]}-${dateSelected[0]} ${event.target.innerText}`,
                };
